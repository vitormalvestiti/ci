...
<head>
  ...
  <link rel="stylesheet" href="<?= base_url('assets/css/pages/job-position/index.css') ?>">
</head>
...
<body>
    ...
    <div class="card shadow-sm">
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <a href="<?= site_url('job_position/create') ?>" class="btn btn-primary">
            <i class="bi bi-plus-circle"></i> Adicionar Cargo
          </a>
        </div>
                ...
                <?php foreach ($job_positions as $job): ?>
                  <tr>
                    ...
                    <td class="text-center">
                      <a href="<?= site_url('job_position/edit/' . $job->id) ?>" class="btn btn-sm btn-outline-warning me-1">
                        <i class="bi bi-pencil-square"></i>
                      </a>
                      <button class="btn btn-sm btn-outline-danger me-1" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal" data-id="<?= $job->id ?>">
                        <i class="bi bi-trash3-fill"></i>
                      </button>
                      <a href="<?= site_url('job_position/assign/' . $job->id) ?>" class="btn btn-sm btn-outline-info">
  ...
  <script>
    ...
    confirmModal.addEventListener('show.bs.modal', function(event) {
      ...
      confirmBtn.href = `<?= site_url('job_position/delete/') ?>${id}`;
    });
  </script>
</body>
